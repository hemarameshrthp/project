FusionCharts.ready(function () {
    var stockPriceChart = new FusionCharts({
        ...
        dataSource: {
            "chart": {
                "caption": "Macine Idle-Time Monitor",
                "exportEnabled":"1",
       ...
       "events": {
           "initialized": function (evt,args) {
               ...
               var dBRef2= firebase.database().ref('idlediff');
               dBRef2.on('value',function(snap){ hema=snap.val()});


               var dBRef4= firebase.database().ref('idlestart');
               dBRef4.on('value',function(snap){ hema1=snap.val()});

               var dBRef6= firebase.database().ref('idlestop');
